runrflasso=function(Y,indice,lag,alpha=1,type="lasso"){
...
rflasso.rolling.window=function(Y,nprev,indice=1,lag=1,alpha=1,type="lasso"){
  
  save.pred=matrix(NA,nprev,1)
  for(i in nprev:1){
    Y.window=Y[(1+nprev-i):(nrow(Y)-i),]
    lasso=runrflasso(Y.window,indice,lag,alpha,type)
